...
SCSFExport scsf_TradingSystemBasedOnLevelsAndAlertCondition(SCStudyInterfaceRef sc)
{
	...
	SCInputRef Input_OrderTextTag = sc.Input[23];
	...
	for (int BarIndex = CalculationStartIndex; BarIndex <= LastIndex; ++BarIndex)
	{
        ...
        // Evaluate only if price crosses any level from the 4H chart
        ...
        {
            double Level = Tool.BeginValue;

            if (sc.High[BarIndex] > Level && sc.Low[BarIndex] < Level)
            {
                SCString MessageText;
                MessageText.Format("crossed level: %.2f", Level);
        ...
        // Skip this bar if no level was crossed
        if (!CrossedAnyLevel)
            continue; // move to next bar in loop
		...
		if (FormulaResult == r_PriorFormulaState)
		{
			...
			if (FormulaResult != 0)
				TradeNotAllowedReason += "True";
			else
				TradeNotAllowedReason += "False";
